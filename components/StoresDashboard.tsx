...
import { StoreAccountModal } from './StoreAccountModal';
...
export const StoresDashboard: React.FC<StoresDashboardProps> = ({
    ...
    const handleConfirmAbono = async () => {
        ...
        // CORRECCIÓN: Calcular saldo pendiente considerando devoluciones
        const returnedValue = (dispatch.returns || []).reduce((acc, r) => {
            const item = dispatch.items.find(i => i.productId === r.productId);
            return acc + (r.quantity * (item?.unitSupplyPrice || 0));
        }, 0);
        const netAmount = dispatch.totalAmount - returnedValue;
        ...
        if (amountNum > pending + 0.01) {
            notify(`ERROR: El monto ($${amountNum.toLocaleString()}) excede el saldo pendiente del folio ($${pending.toLocaleString()}).\n(Despacho: $${dispatch.totalAmount.toLocaleString()}, Devoluciones: $${returnedValue.toLocaleString()}, Pagado: $${paid.toLocaleString()})`, 'error');
            return;
        }
        ...
        try {
            ...
            const paymentId = await onAddStorePayment(payment as StorePayment);
    ...
    const renderPaymentDetail = () => {
        ...
        return (
            <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4" onClick={() => setShowPaymentDetail(null)}>
                <div className="bg-white rounded-md shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
                    <div className="p-8 bg-slate-900 text-white flex justify-between items-center border-b-4 border-[#F97316]">
                        <div>
                            <h4 className="text-xl font-black uppercase tracking-widest">Recibo de Abono</h4>
                            <p className="text-xs font-bold opacity-60 text-[#F97316]">REF: {showPaymentDetail.reference}</p>
                        </div>
                        <button onClick={() => setShowPaymentDetail(null)} className="p-2 bg-white/10 rounded-sm hover:bg-white/20"><ViewIcon className="w-6 h-6 rotate-45" /></button>
                    </div>
                    <div className="p-10 text-center space-y-6 flex-1 overflow-y-auto">
                        <div className="space-y-6">
                            ...
                            {relatedDispatch && (
                                <div className="bg-indigo-50 p-6 rounded-md border border-indigo-100 text-left space-y-4">
                    ...
                    <div className="p-8 bg-gray-50 flex gap-4">
                        <button onClick={() => setShowPaymentDetail(null)} className="flex-1 py-4 font-black uppercase text-[10px] text-gray-400 hover:bg-gray-100 rounded-md transition-all">Cerrar</button>
                        <button onClick={() => handlePrintThermal('payment', showPaymentDetail.id)} className="flex-1 bg-gray-900 text-white py-4 rounded-md font-black text-[10px] uppercase shadow-xl hover:scale-105 transition-all">Imprimir Recibo</button>
                    </div>
                </div>
            </div>
        );
                    ...
                    <div className="p-8 space-y-6 flex-1 overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            ...
                            {showDispatchDetail.items.map((item, idx) => {
                                ...
                                return (
                                    <div key={idx} className="bg-white border border-gray-100 rounded-md p-4 shadow-sm">
                                        ...
                                        <div className="grid grid-cols-12 gap-2 mt-4">
                                            <div className="col-span-4">
                                                <input type="number" placeholder="Cant" value={returnQty[`${showDispatchDetail.id}-${item.productId}`] || ''} onChange={e => setReturnQty({ ...returnQty, [`${showDispatchDetail.id}-${item.productId}`]: e.target.value })} className="w-full p-2 bg-gray-50 border border-gray-200 rounded-sm font-black text-xs outline-none" />
                                            </div>
                                            <div className="col-span-5">
                                                <select value={returnReason[`${showDispatchDetail.id}-${item.productId}`] || 'good_condition'} onChange={e => setReturnReason({ ...returnReason, [`${showDispatchDetail.id}-${item.productId}`]: e.target.value as ReturnReason })} className="w-full p-2 bg-gray-50 border border-gray-200 rounded-sm font-black text-[9px] uppercase">
                                                    <option value="good_condition">Regresa Stock</option>
                                                    <option value="damaged">Dañado (Merma)</option>
                                                    <option value="expired">Vencido</option>
                                                    <option value="lost">Extraviado/Robo</option>
                                                </select>
                                            </div>
                                            <div className="col-span-3">
                                                <button onClick={() => handleItemReturn(showDispatchDetail.id, item.productId)} className="w-full bg-[#F97316] text-white py-2 rounded-sm font-black uppercase text-[9px]">Ajustar</button>
                                            </div>
                                        </div>
                                    </div>
                                );
                    ...
                    <div className="p-8 border-t flex justify-between items-center bg-gray-50">
                        <div>
                         ...
                         {showDispatchDetail.authorizedBy && (
                            <div className="bg-emerald-50 px-4 py-2 rounded-md border border-emerald-200">
    ...
    const renderList = () => (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {stores.map(store => {
                ...
                const isOverdue = store.totalDebt > 0 && storeInvoices.some(d => {
                    if (d.status !== 'active' || !d.dueDate || new Date(d.dueDate) >= new Date()) return false;
                    ...
                    const paid = storePayments
                        ...
                        .reduce((sum, p) => sum + p.amount, 0);
                    // Si el abono está ligado a este despacho, verificamos si aún debe
                    // (Si hay abonos antiguos sin dispatchId, el comportamiento fallback es el totalDebt)
                    return netAmount > paid + 0.05;
                });

                return (
                    <div key={store.id} className="bg-white rounded-md shadow-sm hover:shadow-md hover:border-[#F97316] transition-all p-6 border border-gray-200 relative group overflow-hidden">
                        <div className={`absolute top-0 left-0 w-2 h-full ${isOverdue ? 'bg-red-500 animate-pulse' : 'bg-slate-900'}`}></div>
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h4 className="font-black text-lg text-gray-900 uppercase tracking-tighter truncate leading-none group-hover:text-[#F97316] transition-colors">{store.name}</h4>
                        ...
                        <div className="bg-gray-50 p-4 rounded-md mb-4 border border-gray-100">
                            <div className="flex justify-between items-center">
                                ...
                                {isOverdue && <span className="bg-red-600 text-white text-[8px] px-2 py-0.5 rounded-sm font-black animate-pulse">VENCIDO</span>}
                            </div>
                            <p className="text-2xl font-black text-gray-900 mt-1">${store.totalDebt.toLocaleString()}</p>
                        </div>
                        <button onClick={() => { setActiveStoreId(store.id); setViewMode('statement'); }} className="w-full bg-slate-800 text-white py-3 rounded-md font-black uppercase text-[10px] shadow-sm hover:bg-[#F97316] transition-all">Estado de Cuenta</button>
                    </div>
                );
            })}
        </div>
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
                <div className="flex justify-between items-center mb-8">
                    <div className="flex items-center gap-4">
                        <button onClick={() => setShow360(true)} className="bg-indigo-50 text-indigo-700 px-6 py-3 rounded-xl font-black uppercase text-[10px] flex items-center gap-2 hover:bg-indigo-100 transition-all shadow-sm border border-indigo-100">
                            <ActivityIcon className="w-4 h-4" />
                            Extracto Auditado 360
                        </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-slate-900 text-white p-8 rounded-md md:col-span-2 flex justify-between items-center shadow-2xl border-l-8 border-indigo-500">
                        <div>
                        ...
                        <button onClick={() => setShowAbonoModal(true)} className="bg-emerald-600 hover:bg-emerald-500 text-white px-10 py-5 rounded-md font-black uppercase text-xs shadow-xl shadow-emerald-900/40 tracking-widest transition-all">Registrar Abono</button>
                    </div>
                </div>
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex justify-between items-center mb-8">
                ...
                {isAdmin && (
                    <button onClick={() => setShowEditModal(true)} className="bg-brand-primary text-white px-10 py-4 rounded-md font-black uppercase text-xs tracking-widest shadow-2xl hover:-translate-y-1 transition-all">+ Nueva Sucursal</button>
                )}
            </div>
                    ...
                    <div className="bg-white rounded-md shadow-2xl w-full max-w-lg overflow-hidden border border-gray-200">
                        ...
                        <div className="p-10 space-y-8">
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e => {
                                        ...
                                        let amount = '';
                                        if (dispatch) {
                                            // CORRECCIÓN: Considerar devoluciones en el saldo pendiente
                                            const returnedValue = (dispatch.returns || []).reduce((acc, r) => {
                                                const item = dispatch.items.find(i => i.productId === r.productId);
                                                return acc + (r.quantity * (item?.unitSupplyPrice || 0));
                                            }, 0);
                                            ...
                                            const paid = storePayments
                                                ...
                                                .reduce((sum, p) => sum + p.amount, 0);
                                            amount = Math.max(0, netAmount - paid).toString();
                                        }
                                        setAbonoForm({ ...abonoForm, dispatchId: dId, amount });
                                    }}
                                    className="w-full p-5 bg-gray-50 border-2 border-gray-100 rounded-md font-black text-xs outline-none focus:border-emerald-500"
                                >
                                    <option value="">-- Seleccionar Folio Pendiente --</option>
                                    {dispatches.filter(d => d.storeId === activeStore.id && d.status !== 'cancelled').map(d => {
                                        // CORRECCIÓN: Mostrar saldo pendiente real considerando devoluciones
                                        const returnedValue = (d.returns || []).reduce((acc, r) => {
                                            const item = d.items.find(i => i.productId === r.productId);
                                            return acc + (r.quantity * (item?.unitSupplyPrice || 0));
                                        }, 0);
                                        ...
                                        if (pending <= 0.05) return null; // No mostrar dispatches sin saldo pendiente

                                        return (
                                            <option key={d.id} value={d.id}>
                                                Folio #{d.dispatchNumber} - Saldo: ${pending.toLocaleString()} {returnedValue > 0 ? `(con $${returnedValue.toLocaleString()} dev.)` : ''}
                                            </option>
                                        );
                                    })}
                                </select>
                            </div>
                            <div className="bg-emerald-50 border border-emerald-100 p-8 rounded-md">
                            ...
                            <div>
                                ...
                                <input type="text" value={abonoForm.reference} onChange={e => setAbonoForm({ ...abonoForm, reference: e.target.value })} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-md font-black text-gray-900" placeholder="Ej: Pago Lunes" />
                            </div>
                            <button onClick={handleConfirmAbono} disabled={!abonoForm.dispatchId} className={`w-full ${!abonoForm.dispatchId ? 'bg-gray-400' : 'bg-emerald-600 shadow-xl shadow-emerald-900/20'} text-white py-5 rounded-md font-black uppercase text-xs tracking-widest transition-all`}>Confirmar y Cerrar</button>
            ...
            {showEditModal && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[300] flex items-center justify-center p-6">
                    <div className="bg-white rounded-md shadow-2xl w-full max-w-2xl overflow-hidden border border-gray-200">
                        ...
                        <div className="p-10 space-y-6">
                            <div className="space-y-4">
                                ...
                                <input type="text" value={editForm.name} onChange={e => setEditForm({ ...editForm, name: e.target.value })} className="w-full bg-gray-50 border-2 border-gray-100 p-4 rounded-md font-bold" />
                            </div>
                            <div className="space-y-4">
                                <label className="text-[10px] font-black text-gray-400 uppercase block tracking-widest">Ubicación / Dirección</label>
                                <input type="text" value={editForm.location} onChange={e => setEditForm({ ...editForm, location: e.target.value })} className="w-full bg-gray-50 border-2 border-gray-100 p-4 rounded-md font-bold" />
                            </div>
                            <div className="flex gap-4">
                                <button onClick={() => setShowEditModal(false)} className="flex-1 py-4 text-xs font-black text-gray-400 uppercase">Cerrar</button>
                                <button onClick={() => { onAddStore(editForm); setShowEditModal(false); }} className="flex-1 bg-indigo-600 text-white py-4 rounded-md font-black uppercase text-xs shadow-xl">Guardar Sucursal</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {show360 && activeStore && (
                <StoreAccountModal 
                    store={activeStore}
                    dispatches={dispatches.filter(d => d.storeId === activeStore.id)}
                    storePayments={storePayments.filter(p => p.storeId === activeStore.id)}
                    products={products}
